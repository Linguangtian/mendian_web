...
Page({
    ...
    onLoad: function(t) {
        ...
        var o = t.id,
            s = t.shareid;
        s ? a.gwc_100() : s = 0, a.setData({
            id: o,
            shareid: s
        });
        var n = e.util.url("entry/wxapp/BaseMin", {
            m: "sudu8_page"
        });
        wx.request({
            url: n,
    ...
    getPro: function() {
        var t = this,
            ...
            o = t.data.id,
            s = e.util.url("entry/wxapp/globaluserinfo", {
                m: "sudu8_page"
            });
        wx.request({
            url: s,
    ...
    daojishi: function() {
        for (var a = this, e = a.data.overtime, i = [], o = 0; o < e.length; o++) {
            var s, n, r, u, d = (new Date).getTime(),
                c = 1e3 * parseInt(e[o]) - d;
            0 <= c && (s = Math.floor(c / 1e3 / 60 / 60 / 24), n = Math.floor(c / 1e3 / 60 / 60 % 24) < 10 ? "0" + Math.floor(c / 1e3 / 60 / 60 % 24) : Math.floor(c / 1e3 / 60 / 60 % 24), r = Math.floor(c / 1e3 / 60 % 60) < 10 ? "0" + Math.floor(c / 1e3 / 60 % 60) : Math.floor(c / 1e3 / 60 % 60), u = Math.floor(c / 1e3 % 60) < 10 ? "0" + Math.floor(c / 1e3 % 60) : Math.floor(c / 1e3 % 60)), i[o] = s + "天" + n + ":" + r + ":" + u
        }
        a.setData({
            daojishi: i
    ...
    getproinfo: function() {
        for (var t = this, a = t.data.gzjson, i = t.data.grouparr, o = t.data.id, s = "", n = 0; n < i.length; n++) s += a[i[n]].val[a[i[n]].ck] + "/";
    ...
    gmget: function() {
        ...
        if (c = c.substring(0, c.length - 1), e.ggz = c, 1 == n) var h = 1 * e.price * i;
        else h = 1 * e.dprice * i;
        var p = {};
        p.cid = s.cid, p.id = s.id, p.title = s.title, p.thumb = s.thumb, o.baseinfo2 = p, o.proinfo = e, o.num = i, o.pvid = e.pid, o.one_bili = u.one_bili, o.two_bili = u.two_bili, o.three_bili = u.three_bili, o.gmorpt = n;
        var f = [];
